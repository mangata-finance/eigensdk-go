...
func (ops *OperatorPubkeysServiceInMemory) startServiceInGoroutine(ctx context.Context, queryC <-chan query, wg *sync.WaitGroup) {
	go func() {
		...
		ops.queryPastRegisteredOperatorEventsAndFillDb(ctx, ops.deployBlock, latest, pubkeyDict)
...
func (pkcs *OperatorPubkeysServiceInMemory) queryPastRegisteredOperatorEventsAndFillDb(ctx context.Context, deployBlock uint64, latest uint64, pubkeydict map[common.Address]types.OperatorPubkeys) {
	// Node RPCs usualy limit amout of blocks we can query, iterate from deployment block with LIMIT step size.
	// It doesn't matter if we query some events that we will receive again in the websocket,
	// since we will just overwrite the pubkey dict with the same values.
	for latest > deployBlock {
		end := big.NewInt(int64(latest))
		latest = latest - pkcs.filterQueryLimit
		// uint will wrap-around to max on overflow
		if int64(latest) < int64(deployBlock) {
			latest = deployBlock
		}
		start := big.NewInt(int64(latest))
		alreadyRegisteredOperatorAddrs, alreadyRegisteredOperatorPubkeys, err := pkcs.avsRegistryReader.QueryExistingRegisteredOperatorPubKeys(ctx, start, end)
		if err != nil {
	...
	pkcs.logger.Debug("List of queried operator registration events in blsApkRegistry", "pubkeydict", pubkeydict, "service", "OperatorPubkeysServiceInMemory")
}
